...
const MovieDisplay: React.FC<MovieDisplayProps> = ({ scenesInfo }) => {
  ...
  const transcode = async () => {
    ...
    for (const key in scenesInfo) {
      const filename = `input${key}.mp4`;
      await ffmpeg.writeFile(filename, await fetchFile(scenesInfo[key].url));
    }
  
    // filelist
    const fileList = 'file_list.txt';
    const fileContent = Object.keys(scenesInfo).map((key) => `file 'input${key}.mp4'`).join('\n');
